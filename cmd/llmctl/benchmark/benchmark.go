...
const (
	...
	defaultBenchmarkImage    = "rolebasedgroup-registry.cn-beijing.cr.aliyuncs.com/rolebasedgroup/rbgs-benchmark-tool-genai:v0.1.0"
	defaultCPULimit          = "2"
	defaultCPURequest        = "1"
...
func runBenchmark(ctx context.Context, rbgName string) error {
	...
	rbgClient, err := util.GetControllerRuntimeClient(benchmarkOpts.cf)
	if err != nil {
		return fmt.Errorf("failed to create rbg client: %w", err)
	}
	rbg := &workloadsv1alpha2.RoleBasedGroup{}
	if err := rbgClient.Get(ctx, types.NamespacedName{Name: rbgName, Namespace: ns}, rbg); err != nil {
		return err
	}
