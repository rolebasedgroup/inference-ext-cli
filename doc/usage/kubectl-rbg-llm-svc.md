...
llmctl svc run my-qwen Qwen/Qwen3.5-0.8B --engine sglang
...
llmctl svc run my-qwen Qwen/Qwen3.5-0.8B --resource nvidia.com/gpu=2 --resource memory=16Gi

# Multi-node distributed deployment
llmctl svc run my-qwen Qwen/Qwen3.5-0.8B --distributed-size 4 --shm-size 16Gi
...
Before deploying a model with `llmctl svc run`, you can list available model configurations to discover supported model IDs, run modes, inference engines, and resource presets. Model configs can be built-in or user-defined (placed in `~/.rbg/models/`).
...
llmctl svc chat my-qwen --prompt "What is Kubernetes?"

# Interactive session
llmctl svc chat my-qwen

# With a system prompt
llmctl svc chat my-qwen --system "You are a helpful assistant."
```
...
```bash
llmctl svc run my-model org/new-model \
  --engine vllm \
...
llmctl svc chat my-qwen --prompt "解释 MoE 架构的优势"
...
llmctl svc run my-custom custom-org/custom-model \
  --engine vllm \
...
# 9. Clean up
llmctl svc delete my-qwen my-custom
```
